...
check_setup_needed() {
    ...
    if [ -d "venv" ]; then
        ...
        if ! python -c "import strands, bedrock_agentcore, fastapi, seaborn, Presentation, seaborn, RGBColor, Runtime, pandas" 2>/dev/null; then
...
start_backend() {
    ...
    (
        ...
        if [ ! -f "CoreAgentRuntime.py" ]; then
            echo "❌ CoreAgentRuntime.py not found"
            exit 1
        fi
...
main() {
    ...
    if check_setup_needed; then
        ...
        echo "✅ Setup already done, Not required"
    fi
